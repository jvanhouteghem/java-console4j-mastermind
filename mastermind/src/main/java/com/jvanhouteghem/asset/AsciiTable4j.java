...
public class AsciiTable4j {
	...
	*********************************************************
	 ...
	 * Show Table
	 */
	public String showTable(int tabNumber){
		formatTable(false);
		...
		for (ArrayList<String> currentRow : baseArray) {
			...
			if (j == 0 ){
				outputString +="\n" + addSpaces(tabNumber) + addsStripes(getRowWidth(0), getNumberOfCells(0)); // number of celles
			}
			outputString += "\n";
			j++;
		}
	 ...
	 * @return
	 */
	public static String addSpaces(int nbSpaces){
		String output = "";
